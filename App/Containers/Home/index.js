...
import { View, Text, StyleSheet, FlatList, Dimensions, Platform, ActivityIndicator, TouchableOpacity, ScrollView } from 'react-native';
import {
    RandomUserDataAction
} from '../../redux/actions'
...
function Home(props) {
    ...
    // inital the getRandomUserData function is called.
    useEffect(() => {
        getRandomUserData();
    }, []);
    //pull to refresh
    ...
    // the code of footer that contain the load more pagination concept
    const renderFooter = () => {
        return (
            <View style={styles.footer}>
                <TouchableOpacity
                    activeOpacity={0.9}
                    onPress={() => { getRandomUserData() }}
    ...
    // To add favourite in the list
    const addfavourite = async (detail, index) => {
        props.addFavourite(detail.email)
    }
    //To unfavourite item from the list
    const unfavourite = async (detail, index) => {
        props.unFavourite(detail.email)
    }
    //this func call the api from the redux
    function getRandomUserData(pagination) {
        setLoading(true);
        const {
            RandomUserDataAction
        } = props;
        RandomUserDataAction(offset);
        setLoading(false);
        setRefreshing(false)
        setOffset(offset + 20)
    }

    // render of the flatlist
    function renderItem(item, index) {
        return (
            <View key={index}>
                <View style={styles.Outercard}>
                    <View style={styles.card}>
                        <TouchableOpacity style={{ alignSelf: 'flex-end', margin: 10 }} onPress={() => { item.fav == '0' ? addfavourite(item, index) : unfavourite(item, index) }}>
